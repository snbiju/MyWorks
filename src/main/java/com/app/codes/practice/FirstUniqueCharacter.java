...
public class FirstUniqueCharacter {
    ...
    record Record(int count, int index) {
    }

    public static int findFirst(String str) {
        Map<Character, Record> countMap = new LinkedHashMap<>();

        for (int i = 0; i < str.length(); i++) {
            int index = i;
            char c = str.charAt(index);
            countMap.compute(c, (key, value) -> value == null ? new Record(1, index) : new Record(value.count + 1, value.index()));
        }


        for (Record record : countMap.values()) {
            if (record.count() == 1) {
                return record.index();
            }
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(findFirst("bestcode"));      //  0
        System.out.println(findFirst("lovetocodelots")); // 2
        System.out.println(findFirst("aabb"));//-1
    }
